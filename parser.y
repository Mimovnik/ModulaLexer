...
/* One of the following:
   - identifier,
   - keyword KW_ARRAY, specification of diomensions (DIMEN_SPECS), keyword OF,
     and type specification (TYPE_SPEC)
   - keyword KW_RECORD, fields (FIELDS), keyword END
*/
TYPE_SPEC:
  IDENT
...
/* Either a factor (FACTOR),
   or a pair of simple expressions separated with an operator OP */
SIMPLE_EXPR:
  FACTOR
...
ELSIFS:
  %empty
  | KW_ELSIF EXPR KW_THEN STATEMENTS
;
...
/* CASES */
/* lists of cases (CASE) separated with vertical bars */
CASES:
  CASE
  | CASE '|' CASES
;

/* CASE labels (CASE_LABELS), colon,
